...
describe("Auth Component Test Cases", () => {
  ...
  it("#2 :Should change button name by icon click", async () => {
    ...
    await fireEvent.click(screen.getByTestId("toggle-icon"));
    expect(await screen.findByRole("button")).toHaveTextContent("Register");
  });
  ...
  it("#5 :Should output success msg when register succeeded", async () => {
    render(
      <Provider store={store}>
        <Auth />
      </Provider>
  ...
  it("#6 :Should output error msg when registration failed", async () => {
    server.use(
      rest.post("http://localhost:8000/api/create/", (res, req, ctx) => {
        return res(ctx.status(400));
      })
    );
    render(
